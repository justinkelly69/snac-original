...
export const clone = (snac, settings = {}, newID = false) => {
    ...
    return type === '@' ? (cloneSNAC(snac, settings, newID)
    ) : type === 'N' ? (cloneElement(snac, settings, newID)
...
export const cloneSNAC = (snac, settings = {}) => {
    const newID = has(settings, 'newID', false)
    return ({
        _: newID ? `@${v4()}` : snac._,
        R: cloneElement(snac, settings)
    })
}
...
export const cloneChildren = (kids = [], settings = {}) => {
    ...
    kids.forEach(kid => {
        ...
        if(remove.length > 0 && remove[removeIndex] === kid._) {
            removeIndex = removeIndex + 1
            removeIndex === remove.length &&
            replace.forEach(rep => {
                newKids = [...newKids, clone(rep, { newID: newID })]
...
export const newSnac = (snac = {}) => cloneSNAC(snac, { newID: true })
