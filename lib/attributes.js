export const updateAttributes = (A, ns, name, func1, func2) => {
    ...
    Object.keys(A).sort().forEach(a => {
        ...
        Object.keys(A[a]).sort().forEach(aa => {
            (a === ns) && aa === name ?
...
export const saveAttributes = (atts) => {
    const newAtts = {}
    Object.keys(atts).forEach(a => {
        newAtts[a] = {}
        Object.keys(atts[a]).forEach(aa => 
            (!atts[a][aa].deleted) ?
